...
import getBlogs, { BlogType } from '@/utility/getBlogData'



const getBlogByID = async(id : number) => {
        const blogs = await getBlogs();
        const blogData  = blogs.items
        const data = blogData.find((item : any) => item.id === id)
}

const page = async ({ params }: { params: { id: number } }) => {

    // const url = decodeURIComponent(params.id.replace(/\+/g, " "));
    // const blogs = await getBlogByID(url);
    // console.log(url);
